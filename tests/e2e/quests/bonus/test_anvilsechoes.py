"""Anvil's Echoes stage-by-stage quest coverage."""
import asyncio
...
async def test_anvilsechoes_stage_0_to_1_accept(test_username):
    seed_player(
        test_username,
        position=adjacent_to("blacksmith"),
        inventory=[],
        quests=[{"key": "anvilsechoes", "stage": 0, "subStage": 0, "completedSubStages": []}],
    )
    try:
        ...
        assert_quest_state(test_username, "anvilsechoes", stage=1, sub_stage=0, completed_sub_stages=[])
    finally:
        cleanup_player(test_username)


@pytest.mark.quest_chain
async def test_anvilsechoes_stage_1_to_2_finish(test_username):
    seed_player(
        test_username,
        position=adjacent_to("blacksmith"),
        inventory=[],
        quests=[{"key": "anvilsechoes", "stage": 1, "subStage": 0, "completedSubStages": []}],
    )
    try:
        async with mcp_session(username=test_username) as session:
            result = await session.call_tool("interact_npc", {"npc_name": "Blacksmith"})
            ...
            await asyncio.sleep(1.0)
        await asyncio.sleep(AUTOSAVE_WAIT)
        assert_quest_finished(test_username, "anvilsechoes", stage_count=2)
    finally:
        cleanup_player(test_username)
