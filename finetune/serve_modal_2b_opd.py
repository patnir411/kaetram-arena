"""
Modal serving endpoint for the OPD round-1 Qwen3.5-2B (`2b-opd-r1`).

Serves the merged `kaetram-qwen3.5-2b-opd-r1` checkpoint (reverse-KL on-policy
distillation of base-2B toward the 4B teacher) for the KL gate and the eval run.
Same serving stack and decode config as serve_modal_2b.py so 2b-opd-r1 vs base-2B
...
Usage:
    ...
    # Endpoint: https://workspace--kaetram-qwen-2b-opd-inference-serve.modal.run/v1
"""

import modal

app = modal.App("kaetram-qwen-2b-opd")
...
@app.cls(
    image=serve_image,
    gpu="L4",  # light eval serving; flip to A100 for any batch /v1/score build (L4 OOM-loops at 16K-ctx concurrency)
...
class Inference:
    @modal.enter()
    def load_model(self):
        """Load the merged 2b-opd-r1 checkpoint from the model volume."""
        print(f"Loading 2b-opd-r1 merged checkpoint from {MERGED_PATH}...")
        ...
        print("SGLang engine ready (2b-opd-r1).")
    ...
    def serve(self):
        ...
        async def list_models():
            return {"data": [{"id": "2b-opd-r1", "object": "model"}]}
        ...
        async def chat_completions(request: Request):
            ...
            return {
                ...
                "model": body.get("model", "2b-opd-r1"),
