"""
Modal serving endpoint for the OPD round-3 Qwen3.5-2B (`2b-opd-r3`).

Serves the merged `kaetram-qwen3.5-2b-opd-r3` checkpoint (reverse-KL on-policy
distillation of base-2B toward the 4B teacher) for the KL gate and the eval run.
Same serving stack and decode config as serve_modal_2b.py so 2b-opd-r3 vs base-2B
...
Usage:
    ...
    # Endpoint: https://workspace--kaetram-qwen-2b-opd-r3-inference-serve.modal.run/v1
"""

import modal

app = modal.App("kaetram-qwen-2b-opd-r3")
...
MERGED_PATH = "/checkpoints/kaetram-qwen3.5-2b-opd-r3/merged"
...
class Inference:
    @modal.enter()
    def load_model(self):
        """Load the merged 2b-opd-r3 checkpoint from the model volume."""
        print(f"Loading 2b-opd-r3 merged checkpoint from {MERGED_PATH}...")
        ...
        print("SGLang engine ready (2b-opd-r3).")
    ...
    def serve(self):
        ...
        async def list_models():
            return {"data": [{"id": "2b-opd-r3", "object": "model"}]}
        ...
        async def chat_completions(request: Request):
            ...
            # /score endpoint, and the r1/r2 evals were all actually served, and it
            # matches r3's own training context (opd_2b_data._render renders with
            # add_generation_prompt=True and no enable_thinking). Keeping it off
            # holds the thinking-mode axis constant across base/r1/r2/r3 so a round-3
            # Core-3 delta is attributable to the training round, not to a serve-time
            # mode flip. The checkpoint reasons as plain content + a trailing
            # `</think>` regardless of which prompt primes it (it never saw an open
            # `<think>` in training); enabling thinking here would be an unvalidated
            # train/serve shift, so it stays off until ablated on its own.
            ...
            return {
                ...
                "model": body.get("model", "2b-opd-r3"),
