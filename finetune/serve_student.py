"""
Parameterized Modal serving endpoint for any Qwen3.5 student checkpoint.

One deployable app replaces the per-checkpoint serve clones
(serve_modal_2b.py / serve_modal_2b_opd*.py): the model source, app name, and
reported label are set by environment variables at `modal deploy` time and are
baked into the container image env, so the container resolves the identical
values on import. Serving stack and decode config are byte-identical to the
per-round serves so any arm served here is apples-to-apples with the June 2026
base/r1/r2/r3 evals.

Env vars (read at deploy time):
    KAETRAM_SERVE_APP         Modal app name (default: kaetram-qwen-student).
                              Endpoint URL becomes
                              https://<workspace>--<app>-inference-serve.modal.run/v1
    KAETRAM_SERVE_MODEL_PATH  What SGLang loads. Either a checkpoint-volume path
                              (/checkpoints/<experiment>/merged) or a HuggingFace
                              repo id (Qwen/Qwen3.5-2B, patnir41/kaetram-qwen3.5-2b-opd-r3).
                              Default: Qwen/Qwen3.5-2B (the base student).
    KAETRAM_SERVE_MODEL_LABEL Label reported by /health, /v1/models, and responses
                              (feeds run.meta.json via play_qwen). Default: 2b-student.
    KAETRAM_SERVE_TOKENIZER_ID  Tokenizer + chat-template source. Default
                              Qwen/Qwen3.5-2B — the canonical UNPATCHED base repo;
                              patch_qwen_chat_template is applied in-process (merged
                              checkpoints already carry a patched template, which the
                              patcher would not find its target in; encoding is
                              identical since OPD adds no tokens).

Usage:
    # E3' Arm A (no-seed r2 ablation) from the checkpoint volume:
    KAETRAM_SERVE_APP=kaetram-qwen-2b-opd-r2-noseed \
    KAETRAM_SERVE_MODEL_PATH=/checkpoints/kaetram-qwen3.5-2b-opd-r2-noseed/merged \
    KAETRAM_SERVE_MODEL_LABEL=2b-opd-r2-noseed \
    modal deploy finetune/serve_student.py

    # Reproduce the published r3 from HuggingFace weights (no Modal volume needed):
    KAETRAM_SERVE_APP=kaetram-qwen-2b-opd-r3-hf \
    KAETRAM_SERVE_MODEL_PATH=patnir41/kaetram-qwen3.5-2b-opd-r3 \
    KAETRAM_SERVE_MODEL_LABEL=2b-opd-r3 \
    modal deploy finetune/serve_student.py
"""
...
APP_NAME = os.environ.get("KAETRAM_SERVE_APP", "kaetram-qwen-student")
MODEL_PATH = os.environ.get("KAETRAM_SERVE_MODEL_PATH", "Qwen/Qwen3.5-2B")
MODEL_LABEL = os.environ.get("KAETRAM_SERVE_MODEL_LABEL", "2b-student")
TOKENIZER_ID = os.environ.get("KAETRAM_SERVE_TOKENIZER_ID", "Qwen/Qwen3.5-2B")
# L4 for eval serving; batch /v1/score builds over 16K contexts need A100 —
# the L4 wedges at score concurrency near the context limit (June ops flipped
# GPU for build windows; this makes that a deploy-time switch).
GPU = os.environ.get("KAETRAM_SERVE_GPU", "L4")
...
serve_image = (
    ...
    .env({
        ...
        # Bake the deploy-time parameterization into the container so the
        # module-level constants above resolve identically on container import.
        "KAETRAM_SERVE_APP": APP_NAME,
        "KAETRAM_SERVE_MODEL_PATH": MODEL_PATH,
        "KAETRAM_SERVE_MODEL_LABEL": MODEL_LABEL,
        "KAETRAM_SERVE_TOKENIZER_ID": TOKENIZER_ID,
    })
...
# Matched to serve_modal.py so all arms use an identical decode config.
...
@app.cls(
    image=serve_image,
    gpu=GPU,  # KAETRAM_SERVE_GPU: L4 eval serving, A100 for batch /v1/score builds
...
class Inference:
    @modal.enter()
    def load_model(self):
        """Load the configured checkpoint (volume path or HF repo id)."""
        ...
        print(f"SGLang engine ready ({MODEL_LABEL}).")
    ...
    def serve(self):
        ...
        async def list_models():
            return {"data": [{"id": MODEL_LABEL, "object": "model"}]}
        ...
        async def chat_completions(request: Request):
            ...
            # injects Qwen3.5's native tool spec block. Non-SFT checkpoints
            # were never finetuned on our XML format, so without this they fall
            # back to free-text JSON (markdown-fenced) and `tool_calls` is
            # always empty. The native template reminds the model to emit
            ...
            # Non-thinking generation prompt (Qwen3.5 template default: a CLOSED
            # empty `<think>\n\n</think>\n\n`). This is what base-2B, the 4B teacher
            # /score endpoint, and every r1/r2/r3 eval were actually served; it also
            # matches the OPD training render (opd_2b_data._render uses
            # add_generation_prompt=True and no enable_thinking). Keeping it off
            # holds the thinking-mode axis constant across every arm this file
            # serves, so a Core-3 delta is attributable to the weights, not to a
            # serve-time mode flip.
            ...
            return {
                ...
                "model": body.get("model", MODEL_LABEL),
        ...
        async def score(request: Request):
            ...
            Offline scoring needs the prompt structure to match the
