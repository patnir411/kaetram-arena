...
live teacher/student endpoints and implementing that objective remain explicit
boundaries. The separate
[`corrected-interface SFT adapter`](opd-corrected-interface-sft.md) emits a
pretokenized, render-identity-bound bundle. The SCoRe-style adapter prepares
correction SFT and validates its short-horizon target-reward loss interface,
but fails closed before Stage 2. SCoRe-style Stage 2 and reviewed Modal
execution remain explicit boundaries; the direct-token SFT route is
