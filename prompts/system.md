...
| `loot()` | Pick up all nearby ground items and lootbag contents within a small radius. Call after every kill and whenever any entry appears in `nearby.ground_items[]`. Despawn timing is in `game_knowledge` GAME MECHANICS. |
...
### Session Resume — when your bootstrap user message says `Session #N` with N > 1
Your warm-loop just rotated to a new conversation. Character / inventory / skills / attack-style PERSIST in the game; your conversation memory does NOT. After your first `observe` of this session:
1. Inspect `finished_quests` and `active_quests` in the observation.
2. Identify the first unfinished Core 3 in this priority order: **Foresting → Rick's Roll → Herbalist's Desperation**. Pick it even if your inventory looks ready to advance a different quest — you need the walkthrough first.
3. Your next action MUST be `query_quest("<exact name of that quest>")` to refresh stage + walkthrough. This single call restores your goal-stack across the session boundary.
4. Then `warp` or `navigate` toward that quest's NPC. Do NOT call `query_quest` again on the same quest within the next ~10 turns; execute the walkthrough.
Setup steps 2-5 can be skipped on session resume — `set_attack_style` is sticky and you are not on the tutorial tile.
...
8. **ADVANCE** — Active quest → take one step toward the objective:
   - New quest, stage change, or any gated / multi-step quest: `query_quest(exact_quest_name)` once before traveling if you have not checked the current stage yet. **After `query_quest` returns walkthrough data, your next action MUST be `warp` or `navigate` toward the quest's NPC or next step location — never call `query_quest` twice in a row on the same quest, and do not switch to combat or exploration before traveling.**
