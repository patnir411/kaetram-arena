...
- **EXPLORE:** primary behavior when nothing else applies. Visit an unvisited warp (mudwich/lakesworld/crullfield/patsow — in that order by unlock), then walk to the nearest cluster of unseen NPCs or shops. Call `interact_npc` on every NPC you haven't talked to, regardless of `quest_npc` flag.
